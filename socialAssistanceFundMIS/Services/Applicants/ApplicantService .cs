...
namespace socialAssistanceFundMIS.Services.Applicants
{
    public class ApplicantService : IApplicantService
    {
        ...
        public ApplicantService(ApplicationDbContext context, GeographicLocationService geographicLocationService)
        {
            _context = context;
        ...
        public async Task<List<ApplicantViewModel>> GetAllApplicantsAsync()
        {
            var applicants = await _context.Applicants
                .Include(a => a.PhoneNumbers.Where(p => !p.Removed))
                .Include(a => a.MaritialStatus)
                .Include(a => a.Sex) // Add navigation for Sex if exists
                .Include(a => a.Village) // Add navigation if exists
                .Where(a => !a.Removed)
                .ToListAsync();
            ...
            var applicantsViewModel = applicants.Select(a => new ApplicantViewModel
            {
                ...
                PhoneNumbersListString = string.Join(", ", a.PhoneNumbers.Where(p => !p.Removed).Select(p => p.PhoneNumber))
            }).ToList();
        ...
        public async Task<ApplicantViewModel> UpdateApplicantAsync(int id, Applicant updatedApplicant, List<string> phoneNumbers)
        {
        ...
        private async Task<ApplicantViewModel> MapToViewModelAsync(Applicant applicant)
        {
            ...
            return new ApplicantViewModel
            {
                ...
                PhoneNumbersListString = string.Join(", ", applicant.PhoneNumbers.Where(p => !p.Removed).Select(p => p.PhoneNumber))
            };
        }
